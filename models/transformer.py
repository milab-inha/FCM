...
#         qkv = qkv.view(bs, n_ctx, self.heads, -1).to(torch.float16)
#         q, k, v = torch.split(qkv, attn_ch, dim=-1)

#         attn_output = flash_attn_func(q, k, v, causal=False).to(torch.float32)

#         attn_output = attn_output.reshape(bs, n_ctx, -1)
#         return attn_output
...
class QKVMultiheadAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        with autocast('cuda'):
            attn_output = scaled_dot_product_attention(q, k, v, scale = 1.0)
        
        attn_output = einops.rearrange(attn_output.float(), 'B H S D -> B S H D')
