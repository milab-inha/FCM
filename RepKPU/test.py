...
def test(model, args):
    with torch.no_grad():
        model.eval()
        test_input_path = args.input_dir
        total_cd = 0
        counter = 0
        txt_result = []
        data = PlyData.read('/home/shshin/pointe-DPS/results/recon/sample00.ply')
        ...
        pcd_name = test_input_path.split('/')[-1]
        gt = xyz
        input_pcd = rearrange(xyz, 'n c -> c n').contiguous()
        ...
        save_ply(saved_pcd,os.path.join(save_folder,'upsampled.ply'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--o', default=True, help='using original model')
    parser.add_argument('--flexible', action='store_true', help='aribitrary scale?')
    parser.add_argument('--input_dir', default='/home/shshin/pointe-DPS/results/recon/sample00.ply', type=str, help='path to folder of input point clouds')
    parser.add_argument('--gt_dir', default='/home/shshin/pointe-DPS/results/recon/sample00.ply', type=str, help='path to folder of gt point clouds')
    parser.add_argument('--save_dir', default='pcd', type=str, help='save upsampled point cloud and results')
    parser.add_argument('--ckpt', default='/home/shshin/RepKPU/pugan_best.pth', type=str, help='checkpoints')
